...
async function loadDashboardData() {
    ...
    try {
        const sumRes = await fetch(API_BASE + '/analytics/summary' + qs);
        ...
        const chartRes = await fetch(API_BASE + '/analytics/charts' + qs);
...
document.addEventListener('DOMContentLoaded', async () => {
    // Populate locations
    try {
        const locRes = await fetch(API_BASE + '/get_location_names');
